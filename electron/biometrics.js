// FinanceFlow — Biometric Authentication (TouchID / FaceID / Windows Hello)
...
const BIO_FILE = path.join(app.getPath('userData'), '.financeflow_bio');
const APP_SALT = 'FinanceFlow_Bio_v2_2026_GCM';
...
function decryptFromFile() {
  ...
  try {
    ...
    // Supporta sia v2 (GCM) che v1 legacy (CBC) per migrazione
...
module.exports = {
  ...
  async prompt() {
    if (process.platform === 'darwin') {
      try {
        await systemPreferences.promptTouchID('Sblocca FinanceFlow');
        return true;
      } catch { return false; }
    }
    // Windows Hello
